...
def main(args):
    ...
    fp1, fp2 = open(os.path.join(args.output_path, f'{args.model_path_dir}-covid.csv'), 'w'), open(os.path.join(args.output_path, f'{args.model_path_dir}-non-covid.csv'), 'w')
    ...
    with torch.no_grad():
        ...
        for data in tqdm.tqdm(test_loader):
            if args.test_aug==1:
                ...
                if isinstance(x, list):
                    ...
                    pred, pred_prob = decisionMaker(args, pred)
                    preds.append(pred)
        ...
        outfile.to_csv(f"{args.output_path}/{args.model_path_dir}_{postfix}.csv", index=False)
        if args.evalPerformance:   
            
            preds, labs = np.asarray(preds), np.asarray(labs)
            print("prediction: {} ground-truth: {}".format(preds, labs))
            # f =open("/home2/hyun/kdu/eccv_challenge/CCAT-CT-Detection_220614_joc/val_prediction.txt",'a')
            # f.write("prediction: {} ground-truth: {}\n".format(preds, labs))
            # f.close()
            f = pd.DataFrame({'pred':preds_prob})
            f.to_csv('%s/%s/val_prediction_prob1.csv'%(args.output_path, args.model_path_dir))
...
if __name__ == "__main__":
    ...
    global LOG
    LOG = init_logging(log_file="%s/%s_test_eval.log" % (args.output_path, args.model_path_dir))
    LOG.info(args)
